...
#폰트 설정
fontpath = './datasets/Cafe24Oneprettynight.ttf'  #폰트 경로 불러오기
font_path = './datasets/malgun.ttf'
font = font_manager.FontProperties(fname=font_path).get_name()
rc('font', family=font)
mpl.font_manager._rebuild()

df = pd.read_csv('./crawling_data/cleaned_data.csv', index_col=0) #전처리 된 파일 가져오기
df.dropna(inplace=True)  #결측치 제거하기
print(df.info())
print(df.head())

book_index = df[df['title'] == '완전한 행복'].index[0]  #지정 도서 불러오기
# print(movie_index)
print(df.summary[book_index])
words = df.summary[book_index].split(' ')  #split= 띄어쓰기로 잘린 단어들을 리스트로 만들어준다
print(words)
...
stopwords = ['도서', '작가', '독자', '저자'] #필요없는 단어 제거하기
...
#     .generate(df.cleaned_summary[book_index])
